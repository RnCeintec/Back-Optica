...
export const listDiotrias = async (
  ...
  try {
    ...
    let where:
      ...
      var [result, count] = await createQueryBuilder(Diotrias, "d")
      .select(["d", "p","di"])
      .innerJoin("d.paciente", "p", "d.pacienteId = p.id")
      .innerJoin("d.diotria_id", "di", "d.diotria_id = di.id")
      .where(
        `d.diotria_id Like("%${search}%") or p.documento  Like("%${search}%") or p.rz_social Like("%${search}%")` 
      )
      .orderBy("d.id", "DESC")
    ...
    result = result.filter(diotria => diotria.isActive===true);
