import { Sales, Accesorio, SalesDetails } from "../core/entities";
...
export const listFacturas = async (
  ...
  try {
    ...
    const { fechaInicio, fechaFin, limit, offset, tienda } = req.query;
    ...
    var [facturas, count] = await createQueryBuilder(Facturas, "f")
      .select(["f", "v"])
    ...
    facturas = facturas.filter( factura => (factura.ventas.estado=='anulado' && factura.tipo_comprobante=='nota') || (factura.ventas.estado!='anulado'))
...
export const anularFactura = async (
  ...
  const { comprobante ,motivo } = req.body;
  ...
  //aumentando 1 el correlativo de la nota de credito
  const update = {
